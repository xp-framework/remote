...
class Serializer extends \lang\Object {
  ...
  public function mapping($token, $mapping) {
    if (null !== $mapping) {
      if (!$mapping instanceof SerializerMapping) throw new \lang\IllegalArgumentException(
  ...
  public function valueOf($serialized, $context= array()) {
    ...
    switch ($token) {
      ...
      case 'O': {     // generic objects
        ...
        $members= array();
        try {
          $class= \lang\XPClass::forName(strtr($name, $this->packages[0]));
        ...
        if ($class->isEnum()) {
          if ($size != 1 || 'name' != $this->valueOf($serialized, $context)) {
            throw new \lang\FormatException(sprintf(
