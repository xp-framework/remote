...
class XpProtocolHandler extends \lang\Object implements ProtocolHandler {
  ...
  protected function stringOf($value) {
    if ($value instanceof \lang\reflect\Proxy) {
      $s= 'Proxy<';
  ...
  public function invoke($oid, $method, $args) {
    ...
    $r= $this->sendPacket(
      ...
      array(
        ...
        new ByteCountedString($this->serializer->representationOf(\lang\types\ArrayList::newInstance($args)))
      )
    );
    $this->cat && $this->cat->debugf('<<< %s', $this->stringOf($r));
    return $r;
  ...
  protected function sendPacket($type, $data= '', $bytes= array()) {
    ...
    try {
      ...
      throw new \remote\RemoteException($e->getMessage(), $e);
    }
    
    if (DEFAULT_PROTOCOL_MAGIC_NUMBER != $header['magic']) {
      $this->_sock->close();
      throw new \lang\Error('Magic number mismatch (have: '.$header['magic'].' expect: '.DEFAULT_PROTOCOL_MAGIC_NUMBER);
    }
      ...
      switch ($header['type']) {
        ...
        case REMOTE_MSG_EXCEPTION:
          ...
          if ($reference instanceof \remote\RemoteException) {
            ...
            throw new \remote\RemoteException($reference->getMessage(), $reference);
          } else {
            throw new \remote\RemoteException('lang.XPException', new \lang\XPException($this->stringOf($reference)));
          }
        ...
        case REMOTE_MSG_ERROR:
          ...
          $this->_sock->close();
          throw new \remote\RemoteException($message, new \lang\Error($message));

        default:
          $data= $this->readBytes($header['length']);   // Read all left-over bytes
          $this->cat && $this->cat->debug('<<< Response:', addcslashes($data, "\0..\37!@\177..\377"));
          $this->_sock->close();
          throw new \lang\Error('Unknown message type '.\xp::stringOf($header['type']));
      }
    } catch (\io\IOException $e) {
      throw new \remote\RemoteException($e->getMessage(), $e);
    }
