 ...
 * @purpose  Reflection
 */
class MethodDescription extends \lang\Object {
  public
    $name             = '',
    $returnType       = '',
    $parameterTypes   = null,
  ...
  protected function typeString($arg) {
    return null === $arg ? 'void' : ($arg instanceof \remote\ClassReference ? $arg->referencedName() : $arg);
  }
  ...
  public function classSet() {
    $set= new HashSet(); 
    if ($this->returnType instanceof \remote\ClassReference) $set->add($this->returnType);

    for ($i= 0, $s= sizeof($this->parameterTypes->values); $i < $s; $i++) {
      if (!($this->parameterTypes->values[$i] instanceof \remote\ClassReference)) continue;
